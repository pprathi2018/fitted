...
import {
  clothingApi,
  SearchClothingItemsRequest,
  ClothingItemResponse,
} from '@/lib/api/clothing-item-api-client';

interface UseClothingItemsReturn {
  items: ClothingItemResponse[];
  displayedItems: ClothingItemResponse[];
  isInitialLoading: boolean;
  isFilterLoading: boolean;
  hasMore: boolean;
  error: string | null;
  
  currentRequest: SearchClothingItemsRequest;
  
  search: (request: Partial<SearchClothingItemsRequest>) => Promise<void>;
  loadMore: () => void;
...
export function useClothingItems(): UseClothingItemsReturn {
  ...
  const [currentRequest, setCurrentRequest] = useState<SearchClothingItemsRequest>({
    page: 0,
  ...
  const fetchItems = useCallback(async (request: SearchClothingItemsRequest, append = false) => {
    try {
      const response = await clothingApi.searchClothingItems(request);
  ...
  const search = useCallback(async (partialRequest: Partial<SearchClothingItemsRequest>) => {
    const newRequest: SearchClothingItemsRequest = {
      ...currentRequest,
      ...partialRequest,
      page: 0,
      maxSize: ITEMS_PER_PAGE,
